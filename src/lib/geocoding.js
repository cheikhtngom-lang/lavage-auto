...
import { regionLabel, regionCentroid, getCountry, DEFAULT_COUNTRY } from './countries.js';
...
export async function geocodeQuartierRegion(quartier, regionValue, countryCode = DEFAULT_COUNTRY) {
  const country = getCountry(countryCode) || getCountry(DEFAULT_COUNTRY);
  const region = regionLabel(regionValue, country.code);
  ...
  if (trimmedQuartier) {
    try {
      ...
      const query = [trimmedQuartier, region, country.name].filter(Boolean).join(', ');
      const url = `https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=${country.geocodeCode}&q=${encodeURIComponent(query)}`;
  ...
  const centroid = regionCentroid(regionValue, country.code);
  if (centroid) {
